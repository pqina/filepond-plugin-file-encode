import { DataURIWorker } from './utils/DataURIWorker';
...
const plugin = ({ addFilter, utils }) => {
    ...
    addFilter(
        'DID_CREATE_ITEM', (item, { query }) => {
            ...
            item.extend('getFileEncodeBase64String', () => base64Cache[item.id] ? base64Cache[item.id].data : base64Cache[item.id]);
    ...
    addFilter('COMPLETE_PREPARE_OUTPUT', (file, { item, query }) => 
        new Promise(resolve => {
            ...
            Promise.all((file instanceof Blob ? [{ name:null, file }] : file).map(encode)).then(dataItems => {
